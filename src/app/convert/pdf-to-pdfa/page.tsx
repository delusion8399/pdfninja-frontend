...
export default function Page() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      formData.append("format", "pdfa");

      const response = await fetch(`${config.apiBaseUrl}/pdf/convert/pdf-to-pdfa`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownload = () => {
    ...
    link.download = `${file.name.replace(/\.[^/.]+$/, "")}-converted.pdf`;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transform -rotate-1 mb-12">
          ...
          <p className="text-xl text-black mb-6 max-w-3xl mx-auto leading-relaxed font-medium">
            Convert your PDF files to PDF/A format for long-term archiving and compliance.
          </p>
        </div>
        ...
        {file?.convertedUrl && (
          <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transform rotate-1">
            <h2 className="text-2xl font-bold mb-4 text-black">Converted PDF/A</h2>
            <div className="flex flex-col items-center">
              <div className="w-full max-w-md border-2 border-black p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-black font-medium">
                    {file.name.replace(/\.[^/.]+$/, "")}-converted.pdf
                  </span>
                ...
                <button
                  ...
                  className="w-full py-3 px-4 bg-[#4DCCFF] hover:bg-[#7DDAFF] text-black font-bold border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
                >
                  Download PDF/A
                </button>
              </div>
